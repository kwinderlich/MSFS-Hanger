...
export class WayPointManager {
  ...
  setFlightPlan(points) {
    ...
    this.captureFirstDirect = this.points.length > 0;
    this.resequence();
  }
  ...
  revalidate(lat, long) {
    ...
    this.currentWaypoint.activate();
    this.captureFirstDirect = !!this.currentWaypoint && this.currentWaypoint.id === 1;
    this.resequence();
  ...
  getTarget(
    ...
    // For the very first waypoint, fly *directly to the waypoint itself*
    // until we are genuinely close to it. Using projected leg-capture logic
    // too early makes the target keep moving while the aircraft is still
    // trying to reach WP1, which can produce the circling behaviour Keith saw
    // in live tests.
    if (this.captureFirstDirect && p1 && p1.id === 1) {
      const d1 = getDistanceBetweenPoints(lat, long, p1.lat, p1.long);
      const directArrivalRadiusInKM = 0.25 * KM_PER_NM;
      targets.push({ lat: p1.lat, long: p1.long, direct: true });
      if (d1 <= directArrivalRadiusInKM) {
        this.captureFirstDirect = false;
        this.transition();
        return this.currentWaypoint || p1.next;
      }
      return p1;
    }
