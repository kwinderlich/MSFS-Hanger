...
export async function terrainFollow(autopilot, flightInformation) {
  ...
  const { lat, long, trueHeading, speed, declination, alt: currentAlt, groundAlt } = flightInformation.data;
  let maxElevation, geoPolies;

  // If we do not have local ALOS terrain data configured, degrade gracefully.
  // This keeps terrain follow usable as a soft current-location terrain guard
  // instead of throwing missing-data errors.
  if (!alos?.loaded) {
    const bracketed = ceil((max(Number(currentAlt) || 0, (Number(groundAlt) || 0) + TERRAIN_FOLLOW_SAFETY)) / 100) * 100;
    autopilot.setParameters({
      [ALTITUDE_HOLD]: bracketed,
      [TERRAIN_FOLLOW_DATA]: { geoPolies: [] },
    });
    return;
  }
