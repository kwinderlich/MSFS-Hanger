...
const AutoPilot = reloading(
  ...
  class AutoPilot {
    ...
    async setParameters(params) {
      ...
      // In Hangar's integrated Virtual Pilot path we use the simulator's
      // own heading/altitude autopilot modes for the actual control loop,
      // while Pomax still computes the route targets. That is much more
      // stable in MSFS 2024 than directly driving the ailerons/elevator trim.
      if (!wasEnabled && modes.MASTER) {
        const { AUTOPILOT_MASTER: gameAP } = await api.get(`AUTOPILOT_MASTER`);
        if (USE_SIM_AP) {
          if (gameAP === 0) api.trigger(`AP_MASTER`);
        } else {
          if (gameAP === 1) api.trigger(`AP_MASTER`);
        }
        // now we can safely run our autopilot code.
        this.runAutopilot();
      } else if (wasEnabled && !modes.MASTER && USE_SIM_AP) {
        const { AUTOPILOT_MASTER: gameAP } = await api.get(`AUTOPILOT_MASTER`);
        if (gameAP === 1) api.trigger(`AP_MASTER`);
      }
    }
    ...
    async setTarget(key, value) {
      ...
      if (key === ALTITUDE_HOLD && value !== false) {
        ...
        trim.pitch = pitch;
        if (USE_SIM_AP && this.api.connected) {
          api.set(`AUTOPILOT ALTITUDE LOCK VAR`, value);
          api.trigger(`AP_ALT_HOLD_ON`);
        }
      }
      if (key === ALTITUDE_HOLD && value === false && USE_SIM_AP && this.api.connected) {
        api.trigger(`AP_ALT_HOLD_OFF`);
      }
      ...
      if (key === HEADING_MODE && value === false && USE_SIM_AP && this.api.connected) {
        api.trigger(`AP_HDG_HOLD_OFF`);
      }
