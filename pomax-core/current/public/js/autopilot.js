  ...
  constructor(owner) {
    ...
    Object.keys(AP_OPTIONS).forEach((key) => {
      ...
      e.addEventListener(`click`, () => {
        ...
        const altitudeInput = document.querySelector(`#autopilot .altitude`);
        const headingInput = document.querySelector(`#autopilot .heading`);
        // Special handling for our altitude mode: instead of a
        // boolean, let's make this our desired altitude in feet:
        if (value) {
          if (key === `ALT`) {
            value =
              altitudeInput?.value ?? 1500;
          }
          // Just like we did for ALT, we turn HDG from a boolean into a number:
          if (key === `HDG`) {
            value = headingInput?.value ?? 360;
          }
        }
        if (key === `MASTER` && value) {
          const hasWaypoints = Array.isArray(this.owner?.state?.autopilot?.waypoints) && this.owner.state.autopilot.waypoints.length > 0;
          if (hasWaypoints) {
            const flightData = this.owner?.state?.flightInformation?.data || {};
            const payload = { LVL: true };
            const hdg = Number(headingInput?.value ?? flightData.heading);
            const alt = Number(altitudeInput?.value ?? flightData.alt);
            if (Number.isFinite(hdg) && hdg > 0) payload.HDG = hdg;
            if (Number.isFinite(alt) && alt > 0) payload.ALT = alt;
            server.autopilot.update(payload);
          }
        }
    ...
    const trail = domAP.querySelector(`.trail`);
    if (trail) {
      trail.addEventListener(`click`, ({ target: btn }) => {
        trail.classList.toggle(`active`);
        this.owner?.setTrailVisible?.(btn.classList.contains(`active`));
      });
      this.owner?.setTrailVisible?.(trail.classList.contains(`active`));
    }
